...
#define NET_HEIGHT 160
#define NET_WIDTH 160
#define OBJECTNESS_THRESHOLD 0.3
#define IOU_THRESHOLD 0.1

#define YOLO_PICO_INPUT_BYTES 160*160*3
...
#define BBOX_MAX 16
#define BBOX_CMD_ADDRESS   0x01700000
...
namespace {
   ...
   float anchors[3][TOTAL_ANCHORS * 2] = {
       {10, 13, 16, 30, 33, 23},    // 小尺度 (5x5)
       {30, 61, 62, 45, 59, 119}, // 中尺度 (10x10)
       {116, 90, 156, 198, 373, 326} // 大尺度 (20x20)
   };
}
...
void tinyml_init() {
   ...
   MicroPrintf("Output shape: %d %d %d %d\n\r",
       output->dims->size, output->dims->data[0], output->dims->data[1], output->dims->data[2]);
...
extern "C"  void main() {
   ...
   while(1) {
	  ...
	  for (unsigned int i = 0; i < YOLO_PICO_INPUT_BYTES; ++i)
		  model_input->data.int8[i] = tinyml_input_array[i] - 128;
	  ...
	  const int zero_point = params.zero_point->data[0];

	  // 总元素数 = 1 * 1575 * 6
	  int total_elements = output->dims->data[1] * output->dims->data[2];
	  float* output_float = (float*)calloc(total_elements, sizeof(float));
	  for (int i = 0; i < total_elements; ++i) {
		  output_float[i] = ((float)output->data.int8[i] - zero_point) * scale;
	  }

	  // 三个尺度的网格大小
	  const int grids[3][2] = {{5,5}, {10,10}, {20,20}};
	  // 每个尺度的元素数 = grid_h * grid_w * 3 * 6  （3 anchors * 6 个值）
	  int offsets[3];
	  offsets[0] = 0;
	  offsets[1] = grids[0][0] * grids[0][1] * 3 * 6;
	  offsets[2] = offsets[1] + grids[1][0] * grids[1][1] * 3 * 6;

	  // 构建三个 layer，调用 perform_inference
	  layer yolo_layers[3];
	  for (int i = 0; i < 3; ++i) {
		  int h = grids[i][0];
		  int w = grids[i][1];
		  yolo_layers[i].channels = 3;          // 每个网格3个anchor
		  yolo_layers[i].height = h;
		  yolo_layers[i].width = w;
		  yolo_layers[i].classes = CLASSES;     // 1
		  yolo_layers[i].boxes_per_scale = 1;   // 每个anchor一组值，6 = 1*(5+1)
		  yolo_layers[i].total_anchors = TOTAL_ANCHORS; // 3
		  yolo_layers[i].scale = i;
		  yolo_layers[i].anchors = anchors[i];

		  int layer_total = h * w * 3 * 6;      // 这个尺度的float个数
		  yolo_layers[i].outputs = (float*)calloc(layer_total, sizeof(float));
		  memcpy(yolo_layers[i].outputs, &output_float[offsets[i]], layer_total * sizeof(float));
	  }
