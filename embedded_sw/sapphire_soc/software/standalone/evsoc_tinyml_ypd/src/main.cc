...
// Detection thresholds (rational: numerator/denominator)
#define CONF_THR_NUM  20    // Confidence: 3/10 = 30%
#define CONF_THR_DEN  100
#define IOU_THR_NUM   8    // IoU: 1/10 = 0.1
#define IOU_THR_DEN   100
...
// Convert IEEE 754 float to Q20 fixed-point using only integer bit ops
static int32_t f32_to_q20(float f){
    union { float f; uint32_t u; } u = {.f = f};
    uint32_t bits = u.u;
    if(bits==0)return 0;
    int sign = (int)(bits>>31);
    int exp  = (int)((bits>>23)&0xFF)-127;
    int32_t mant = (int32_t)(bits&0x7FFFFF)|(1<<23);
    int shift = exp - 3;  // +20(Q20) -23(mant bits) = -3
    int64_t v;
    if(shift>=0) v = (int64_t)mant<<shift;
    else         v = ((int64_t)mant + (1LL<<(-shift-1)))>>(-shift);
...
extern "C" void main(){
    ...
    // Dequant params → integer fixed-point (no float ops)
    ...
    int32_t scale_q20 = f32_to_q20(oq->scale->data[0]);  // o_sc in Q20

    int conf_thr_i = o_zp + (CONF_THR_NUM<<20)/(CONF_THR_DEN*scale_q20);

    // Pre-combined scale factors for box dequant
    int64_t box_sx = (int64_t)scale_q20 * 128 * nw;
    int64_t box_sy = (int64_t)scale_q20 * 128 * nh;
    ...
    while(1){
        ...
        for(int i=0;i<tot&&nd<MAX_DET;i++){
            ...
            int c=r[4]; if(c<conf_thr_i)continue;

            // Integer box dequant: (2*dc - dw) * o_sc * 128 * nw  →  x1 (256-scaled)
            int dx=(int)r[0]-o_zp, dy=(int)r[1]-o_zp;
            int dw=(int)r[2]-o_zp, dh=(int)r[3]-o_zp;

            int x1=(int)(((2*dx-dw)*box_sx)>>20);
            int y1=(int)(((2*dy-dh)*box_sy)>>20);
            int x2=(int)(((2*dx+dw)*box_sx)>>20);
            int y2=(int)(((2*dy+dh)*box_sy)>>20);
        ...
        for(int i=0;i<nd;i++){
            ...
            for(int j=i+1;j<nd;j++){
                ...
                if(inter*IOU_THR_DEN>(area_a+area_b-inter)*IOU_THR_NUM) supp[j]=1;
            }
        }
        ...
        for(int i=0;i<nk&&i<10;i++){
            ...
            // Pixel coordinates: px = x1/256/nw*CAM_W  (integer)
            int px = (int)(((int64_t)x1*CAM_W)/(256*nw));
            int py = (int)(((int64_t)y1*CAM_H)/(256*SUB_H));
            int pw = (int)(((int64_t)(x2-x1)*CAM_W)/(256*nw));
            int ph = (int)(((int64_t)(y2-y1)*CAM_H)/(256*SUB_H));

            // Confidence percentage: (c - zp) * scale * 100
            int pct = (int)(((int64_t)((int)scores_i[keep[i]]-o_zp)*scale_q20*100)>>20);
