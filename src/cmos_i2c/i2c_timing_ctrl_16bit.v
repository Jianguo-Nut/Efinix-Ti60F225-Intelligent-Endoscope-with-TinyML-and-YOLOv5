...
//----------------------------------------
//gain_update_en rising edge detection
reg gain_update_en_dly;
wire gain_update_en_rise;

always@(posedge clk or negedge rst_n)
begin
    if(!rst_n)
        gain_update_en_dly <= 1'b0;
    else
        gain_update_en_dly <= gain_update_en;
end

assign gain_update_en_rise = gain_update_en & ~gain_update_en_dly;
...
begin
    if(!rst_n)
        ...
        restart_flag <= 1'b0;
    else if(gain_update_en_rise && i2c_config_done)
        restart_flag <= 1'b1;
    else if(i2c_transfer_en && current_state == I2C_IDLE && restart_flag)
        restart_flag <= 1'b0;
end
...
begin
    if(!rst_n)
        ...
        begin
        // Reset index when restart is triggered
        i2c_config_index <= 0;
        end
    else
        i2c_config_index <= i2c_config_index;
        ...
        if(delay_done == 1'b1)  //1ms Setting time after software/hardware reset    
            ...
            if(i2c_transfer_en)
                ...
                if(i2c_config_index < i2c_config_size)
                    ...
                    next_state = I2C_WR_START;  //Write Data to I2C
                else if(restart_flag)  // Add restart condition
                    next_state = I2C_WR_START;  //Restart I2C transmission
