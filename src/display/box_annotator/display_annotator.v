   ...
   if (rst) begin
      ...
      case (state)
         ...
         begin
            ...
            // TLAST 优先：结束时直接回 IDLE，避免 SKIP 状态卡死
            state    <= (in_ready & in_valid & in_last) ? S_IDLE :
                       (in_ready & in_valid & (counter == IMG_DATA_COUNT-1)) ? S_SKIP : S_IMAGE;
         end
         S_BBOX:
         begin
            counter  <= (in_ready & in_valid) ? counter + 1'b1 : counter;
            state    <= (in_ready & in_valid & in_last) ? S_IDLE :
                       (in_ready & in_valid & (counter == MAX_BBOX-1)) ? S_SKIP : S_BBOX;
         end
         S_LOGO:
         begin
            counter  <= (in_ready & in_valid) ? counter + 1'b1 : counter;
            state    <= (in_ready & in_valid & in_last) ? S_IDLE :
                       (in_ready & in_valid & (counter == LOGO_DATA_COUNT-1)) ? S_SKIP : S_LOGO;
         end
